...
//      DONE (6) Move the cursor to the appropriate position
        boolean hasPosition = mCursor.moveToPosition(position);
//      DONE (7) Generate a weather summary with the date, description, high and low
        if(hasPosition){
            ...
            int highTemperatureIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
            int lowTemperatureIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);
            long unixDate = mCursor.getLong(dateIndex);
            String date = SunshineDateUtils.getFriendlyDateString(mContext,unixDate,false);
            int descriptionCode = mCursor.getInt(descriptionIndex);
            String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext,descriptionCode);
            double low = mCursor.getDouble(lowTemperatureIndex);
            double high = mCursor.getDouble(highTemperatureIndex);
            String lowHigh = SunshineWeatherUtils.formatHighLows(mContext,high,low);
//      DONE (8) Display the summary that you created above
            String weatherForThisDay = date + " - " + description + " - " + lowHigh;
